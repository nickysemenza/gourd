...
var (
	...
	usdaLabMethodColumnsWithoutDefault = []string{"id", "description", "technique"}
	usdaLabMethodColumnsWithDefault    = []string{}
...
func (o *UsdaLabMethod) LabMethodUsdaLabMethodCodes(mods ...qm.QueryMod) usdaLabMethodCodeQuery {
	...
	query := UsdaLabMethodCodes(queryMods...)
	queries.SetFrom(query.Query, "\"usda_lab_method_code\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"usda_lab_method_code\".*"})
	}
...
func (o *UsdaLabMethod) LabMethodUsdaLabMethodNutrients(mods ...qm.QueryMod) usdaLabMethodNutrientQuery {
	...
	query := UsdaLabMethodNutrients(queryMods...)
	queries.SetFrom(query.Query, "\"usda_lab_method_nutrient\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"usda_lab_method_nutrient\".*"})
	}
...
func (o *UsdaLabMethod) LabMethodUsdaSubSampleResults(mods ...qm.QueryMod) usdaSubSampleResultQuery {
	...
	query := UsdaSubSampleResults(queryMods...)
	queries.SetFrom(query.Query, "\"usda_sub_sample_result\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"usda_sub_sample_result\".*"})
	}
...
func UsdaLabMethods(mods ...qm.QueryMod) usdaLabMethodQuery {
	...
	return usdaLabMethodQuery{NewQuery(mods...)}
}
